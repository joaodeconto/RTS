		...
		public string Import_CSV( string Category, string CSVstring, eSpreadsheetUpdateMode UpdateMode = eSpreadsheetUpdateMode.Replace, char Separator = ',' )
		{
			...
			for (int i=1, imax=CSV.Count; i<imax; ++i)
			{
				...
				if (termData==null)
				{
					...
					termData.Flags = new int[ mLanguages.Count ];
				...
				for (int j=0; j<LanIndices.Length && j<Tokens.Length-LanguagesStartIdx; ++j)
					if (!string.IsNullOrEmpty(Tokens[j+LanguagesStartIdx]))	// Only change the translation if there is a new value
					{
						...
						if (isAuto) value = value.Substring("[isauto]".Length );

						var lanIdx = LanIndices[j];
						if (isTouch)
						{
							termData.Languages_Touch[ lanIdx ] = value;
							if (isAuto)  termData.Flags[lanIdx] |= (int)TranslationFlag.AutoTranslated_Touch;
									else termData.Flags[lanIdx] &= ~(int)TranslationFlag.AutoTranslated_Touch;
						}
						else
						{
							termData.Languages[ lanIdx ] = value;
							if (isAuto)  termData.Flags[lanIdx] |= (int)TranslationFlag.AutoTranslated_Normal;
									else termData.Flags[lanIdx] &= ~(int)TranslationFlag.AutoTranslated_Normal;
						}
					}
			}
