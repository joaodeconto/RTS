...
namespace I2.Loc
{
	public partial class LocalizationEditor
	{
		...
		void OnGUI_KeysList(bool AllowExpandKey = true, float Height = 300.0f, bool ShowTools=true)
		{
			...
			//--[ Keys List ]-----------------------------------------
			mScrollPos_Keys = GUILayout.BeginScrollView( mScrollPos_Keys, true, false, "horizontalScrollbar", "verticalScrollbar", "AS TextArea", GUILayout.MinHeight(Height), GUILayout.MaxHeight(Screen.height), GUILayout.ExpandHeight(false));
...
//				EditorGUILayout.HelpBox("Use (Tools\\Parse Terms) to find how many times each of the Terms are used", UnityEditor.MessageType.Info);
			...
			if (ShowTools)
			{
				...
				GUI.enabled = (mSelectedKeys.Count>0 || !string.IsNullOrEmpty(mKeyToExplore));
					if (GUILayout.Button ("Add Terms to Source")) 		 AddTermsToSource();
					if (GUILayout.Button ("Remove Terms from Source")) 	 RemoveTermsFromSource();
		...
		void OnGUI_KeyHeader (string sKey, string sCategory, string FullKey, int nUses, float YPosMin)
		{
			...
			if (sCategory != LanguageSource.EmptyCategory) 
			{
				...
				else
				{
					...
					rectKey.x = listWidth - rectKey.width - 38;
		...
		void OnGUI_Keys_ListSelection()
		{
			...
			{ 
				mSelectedKeys.Clear(); 
				foreach (var kvp in mParsedTerms)
					if (!mLanguageSource.ContainsTerm( kvp.Key ))
						mSelectedKeys.Add ( kvp.Key );
			}
			GUI.enabled = true;
		...
		public bool ShouldShowTerm (string FullTerm)
		{
			...
			return ShouldShowTerm (termData.Term, termData.Category, termData.Usage);
		}
