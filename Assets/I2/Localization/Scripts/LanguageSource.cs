...
namespace I2.Loc
{
	...
	public class TermData
	{
		...
		public int[]     Flags 			= new int[0];	// flags for each translation
		...
		{
			if (IsTouch)
				return (Flags[idx] & (int)TranslationFlag.AutoTranslated_Touch) > 0;
			else
				return (Flags[idx] & (int)TranslationFlag.AutoTranslated_Normal) > 0;
		}
	...
	public partial class LanguageSource : MonoBehaviour
	{
		...
		public  TermData AddTerm( string NewTerm, eTermType termType )
		{
			...
			if (data==null) 
			{
				...
				data.Flags = new int[ mLanguages.Count ];
				mTerms.Add (data);
