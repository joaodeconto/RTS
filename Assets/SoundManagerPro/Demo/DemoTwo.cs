...
public class DemoTwo : MonoBehaviour {
	...
	void Awake () {
		if(GameObject.FindObjectsOfType(typeof(DemoTwo)).Length > 1)
		{
			Destroy(gameObject);
			return;
		}
	...
	void OnGUI()
	{
		...
		{
			// You can use SoundManager.Load(clipname, customPath) -- Or set SoundManager.RESOURCES_PATH and forget the custom path
